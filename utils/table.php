...
class App_notif_Table_Users_List extends WP_List_Table {
	...
	function admin_header() {
		...
		if( 'wp-app-notif-users' != $page ) return; 
	...
	function prepare_items() {
		...
		$total_items = app_notif_data_count_users($search);
		$this->items = app_notif_data_get_users($orderby, $order, $item_per_page, $paged, $search);
...
class App_notif_Table_Logs_List extends WP_List_Table {
	...
	function admin_header() {
		$page = ( isset($_GET['page'] ) ) ? esc_attr( $_GET['page'] ) : false;
		if( 'wp-app-notif-history' != $page ) return; 
	...
	function process_bulk_action() {
		...
		if( 'delete'===$this->current_action() ) {			
			...
			$app_notif_logs = $wpdb->prefix . 'app_notif_logs';	
			foreach($array_data as $key => $id_value){ 				
                $wpdb->query( "DELETE FROM $app_notif_logs WHERE id = ".$id_value);				
			}			
	...
	function prepare_items() {
		...
		$total_items = app_notif_data_count_logs($search);
		$this->items = app_notif_data_get_logs($orderby, $order, $item_per_page, $paged, $search);
