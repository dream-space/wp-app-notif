<?php
	$app_notif_table_logs = new App_notif_Table_Logs_List();
	if( isset($_POST['s']) ){
		$app_notif_table_logs->prepare_items($_POST['s']);
	} else {
		$app_notif_table_logs->prepare_items('');
	}
?>
<div class="wrap">
	<h2 class='opt-title'><span id='icon-options-general' class='app-notif-options'>
		...
		<?php echo __( 'APP Notif Logs History', 'app_notif' ); ?>
	</h2>
	
	<!--Search box display -->
	<form action="" method="post">
    <?php 
		$app_notif_table_logs->search_box( __( 'Search' ), 'app_notif' ); 
	...
	<form id="app_notif_Logs_table_list" action="" method="get">
		...
		<?php $app_notif_table_logs->display(); ?>
	</form>
